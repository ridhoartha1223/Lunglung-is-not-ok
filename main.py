...
# -------------------- UTILITY --------------------
def text_to_shape_paths(text: str, font_path: str, size=512):
    """
    Convert text to placeholder shape paths for .tgs
    (In production, should extract glyph contours from font)
    """
...
def generate_lottie(text, font_color=(0,0,0), size=512, animation="fade_in", font_file="Arial.ttf"):
    r,g,b = [c/255 for c in font_color]
    path = text_to_shape_paths(text, os.path.join(FONT_DIR, font_file), size=size)
    ...
    layer = {
        ...
        "shapes":[{"ty":"gr","it":[{"ty":"sh","ks":{"k":path}},{"ty":"fl","c":[r,g,b,1]}]}]
    }
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lottie_json = generate_lottie(text, font_color=color, size=size, animation=anim, font_file=font_file)
